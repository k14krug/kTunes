...
tracks_cur.execute('update tracks set repeat_cnt = 1, last_played=0;')
...
def open_genre_track_cursors():
  ...
  latest_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                          from tracks where genre = 'Latest' and last_played <= ? order by last_play_dt''',(cnt,))
  in_rot_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                          from tracks where genre = 'In Rot' and last_played <= ? order by last_play_dt''',(cnt,))
  other_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                          from tracks where genre = 'Other' and last_played <= ? order by last_play_dt''',(cnt,))
  album_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                          from tracks where genre = 'Album' and last_played <= ? order by last_play_dt''',(cnt,))
  old_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                          from tracks where genre = 'Old' and last_played <= ? order by last_play_dt''',(cnt,))
...
def fetch_genre_cur():
  ...
  if genre == 'Latest':
    ...
    try:
      ...
      debug_out(2,["fetch_genre_cur - Processed all latest tracks, need to start over ",cnt, genre, return_val])
      #debug_out(2,["                  Resetting lyypast_played to 0 for all latest tracks:",cnt, genre, return_val])
      latest_cur.execute('update tracks set last_played=0 where genre = ?;',(genre,))
      latest_cur.execute('select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location from tracks where genre= ? and last_played <= ? order by last_play_dt',(genre, cnt,))
      song,artist, last_play_dt, last_played, rating, length, repeat_cnt, location=latest_cur.fetchone()
    ...
    if check_artist_repeat() == True:
      ...
      latest_cur.execute('''update tracks set last_played=?
                             where artist= ?
                               and genre = ?
      ...
      latest_cur.execute('''update tracks set last_played=999999
                             where song = ?
                               and artist = ?
                               and genre = ?;
                         ''',(song,artist,genre))
  if genre == 'In Rot':
    ...
    if check_artist_repeat() == True:
      ...
      in_rot_cur.execute('''update tracks set last_played=?
                             where artist= ?
                               and genre = ?
                               and last_played <> 999999;
                         ''',(artist_last_played+genre_repeat+1,artist,genre))
      in_rot_cur.execute('''select song, artist, last_play_dt, last_played, rating, length, repeat_cnt, location 
                              from tracks where genre = ? and last_played <= ? order by last_play_dt''',(genre,cnt,))
      #in_rot_cur.execute('select * from in_rot_tracks where last_played <= ? order by last_play_dt',(cnt,))
      song,artist, last_play_dt, last_played, rating, length, repeat_cnt, location=in_rot_cur.fetchone() 
      ...
      in_rot_cur.execute('''update tracks set last_played=999999
                             where song = ?
                               and artist= ?
                               and genre = ?;
                         ''',(song,artist,genre))
  if genre == 'Other':
    ...
    if check_artist_repeat() == True:
      # We failed the artist repeat so set last_played so this artist won't be included till its time again
      other_cur.execute('''update tracks set last_played=?
                             where artist = ?
                               and genre = ?
                               and last_played <> 999999;
                         ''',(artist_last_played+genre_repeat+1,artist,genre))
      ...
      other_cur.execute('''update tracks set last_played=999999
                             where song = ?
                               and artist= ?
                               and genre = ?;
                         ''',(song,artist,genre))
  if genre == 'Album':    
    ...
    if check_artist_repeat() == True:
      # We failed the artist repeat so set last_played so this artist won't be included till its time again
      album_cur.execute('''update tracks set last_played=?
                             where artist = ?
                               and genre = ?
                               and last_played <> 999999;
                         ''',(artist_last_played+genre_repeat+1,artist,genre))
      ...
      album_cur.execute('''update tracks set last_played=999999
                             where song = ?
                               and artist = ?
                               and genre = ?;
                         ''',(song,artist,genre))
  if genre == 'Old':
    ...
    if check_artist_repeat() == True:
      # We failed the artist repeat so set last_played so this artist won't be included till its time again
      old_cur.execute('''update tracks set last_played=?
                             where artist = ?
                               and genre = ?
                               and last_played <> 999999;
                         ''',(artist_last_played+genre_repeat+1,artist,genre))
      ...
      old_cur.execute('''update tracks set last_played=999999
                             where song = ?
                               and artist = ?
                               and genre = ?;
                         ''',(song,artist,genre))
